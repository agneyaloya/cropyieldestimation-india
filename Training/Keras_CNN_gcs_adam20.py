...
X_train = X_shuffled[0:1200, : ,:, :]
Y_train = Y_shuffled[0:1200].reshape(1200, 1)

X_val = X_shuffled[1200:1338, : ,:, :]
Y_val = Y_shuffled[1200:1338].reshape(138, 1)

X_test = X_shuffled[1339:1477, : ,:, :]
Y_test = Y_shuffled[1339:1477].reshape(138, 1)
...
model.compile(loss=keras.losses.mean_squared_error, optimizer = opt_adam, metrics=['mape'])
...
f = open('images/run20/output_Adam.txt','w')
...
'''
mode_train = stats.mode(y_pred_train)

print('mode of prediction on training set', mode_train)
l = len(Y_train)

bad_data = np.zeros(len(Y_train))
j = 0	#counter variable

for i in range(l):
	if(y_pred_train[i] == mode_train[0]):
		bad_data[j] = i
		j += 1
'''


model.save('images/run20/Keras_CNN_gcs_adam_run20.h5')
'''
...
#pp.savefig()
plt.savefig('images/run20/YpredvsYactual_Simpler_Adam.png')
#fig1.show()
...
plt.title("Predicted vs Actual Yield for Validation Set")
#pp.savefig()
plt.savefig('images/run20/YpredvsYactual_Scatter_Adam.png')
...
#pp.savefig()
plt.savefig('images/run20/YpredvsYactualtrain_Scatter_Adam.png')
...
# plt.ylim( 0, 300 )
#pp.savefig()
plt.savefig('images/run20/MAPE_simpler_Adam.png')
...
#pp.savefig()
plt.savefig('images/run20/RMSE_simpler_Adam.png')
#plt.ylim(0,2)
...
plt.savefig('images/run20/Loss_history_trainNval_Adam.png')

#Import CSV
### Output the actual Y, and the Predicted Y for Validation
predictions = np.concatenate((Y_val, y_pred_val), axis = 1)
myFile = open('images/run20/val_predictions_Adam.csv', 'w')
with myFile:
...
#Import CSV
### Output the actual Y_train, and the Predicted Y for train
predictions = np.concatenate((Y_train, y_pred_train), axis = 1)
myFile = open('images/run20/train_predictions_Adam.csv', 'w')
with myFile:
    writer = csv.writer(myFile)
    writer.writerows(predictions)
print("Writing complete")
...
myFile = open('images/run20/justS_Adam.csv', 'w')
with myFile:
    writer = csv.writer(myFile)
    writer.writerows(s)
print("Writing complete")
