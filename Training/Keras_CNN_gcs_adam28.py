...
X_train = X_shuffled[0:1099, : ,:, 0:6]
Y_train = Y_shuffled[0:1099].reshape(1099, 1)

X_val = X_shuffled[1099:1211, : ,:, 0:6]
Y_val = Y_shuffled[1099:1211].reshape(112, 1)

X_test = X_shuffled[1211:1318, : ,:, 0:6]
...
batch_size = 25
epochs = 100
img_rows, img_cols, channels = 32, 32, 6  # input image dimensions
...
model.add(BatchNormalization())
model.add(Flatten())
model.add(Dense(32, activation='relu', kernel_regularizer=regularizers.l2(0.11)))
model.add(Dropout(0.1))
model.add(Dense(1))


#opt_adadelta = keras.optimizers.Adadelta(lr=1.0, rho=0.95, epsilon=None, decay=0.0)
opt_adam = keras.optimizers.Adam(lr=0.0002, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.005, amsgrad=False)
...
f = open('images/run28/output_Adam.txt','w')
...
model.save('images/run28/Keras_CNN_gcs_adam_run28.h5')
...
#pp.savefig()
plt.savefig('images/run28/YpredvsYactual_Simpler_Adam.png')
#fig1.show()
...
plt.title("Predicted vs Actual Yield for Validation Set")
#pp.savefig()
plt.savefig('images/run28/YpredvsYactual_Scatter_Adam.png')
...
#pp.savefig()
plt.savefig('images/run28/YpredvsYactualtrain_Scatter_Adam.png')
...
# plt.ylim( 0, 300 )
#pp.savefig()
plt.savefig('images/run28/MAPE_simpler_Adam.png')
...
#pp.savefig()
plt.savefig('images/run28/RMSE_simpler_Adam.png')
#plt.ylim(0,2)
